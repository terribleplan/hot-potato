...
var mostRecent = "12:terribleplan";
function pass_the_potato(potato) {
    var temp = {};
    try {
        temp = bencode.decode(potato);
    } catch (e) {
        try {
            temp = JSON.parse(potato);
            mostRecent = bencode.encode(temp);
        } catch (e) {
            temp = {"message": "Some idiot sent me non-bEncode"};
        }
    }
    temp.via = ["terribleplan"];

    var beerCount = 100;
    if (temp["bottles_of_beer"]) {
        parseInt(temp["bottles_of_beer"]);
    } else if (temp.beer) {
        parseInt(temp.beer);
    }
    beerCount--;
    temp["bottles_of_beer"] = beerCount;
    temp["beer"] = beerCount + " Bottles of beer on the wall "+beerCount+" bottles of beer, take one down and pass it around, "+(beerCount-1)+" bottles of beer on the wall";
    mostRecent = bencode.encode(temp);
    ...
    for(var i = 1; i !== 257; ++i){
        ...
        if(connections_by_octet[attempted_octet]){
            ...
            connections_by_octet[attempted_octet].write(mostRecent);
            return;
        }
    }
